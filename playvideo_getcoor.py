...
def draw_circle(img,coor_arr):
    for i in range(len(coor_arr)):
        img = cv2.circle(img, (coor_arr[i][0],coor_arr[i][1]), half_track_width, (0,255,225), -1)
    return img
...
def draw_and_save_circle_link_poly(img,coor_a,coor_b,index,binarization_arr,open_save): # get two crycle coordinate and let them link together
    ...
    #draw line
    cv2.line(img, (link_dot[0][0], link_dot[0][1]), (link_dot[3][0], link_dot[3][1]), (0,255,225), 5)
    cv2.line(img, (link_dot[1][0], link_dot[1][1]), (link_dot[2][0], link_dot[2][1]), (0,255,225), 5)
    ...
    if open_save == 1:
        ...
        for i in range(mi_x,ma_x):
            for j in range(mi_y,ma_y):
...
def save_crycle_to_3_array(binarization_arr,coor_arr,index):
    binarization_arr.append([[0 for j in range(hight)]for i in range(width)]) #[x][y]
    ...
    for i in range(x_begin,x_end):
        for j in range(y_begin,y_end):
            ...
            if line_length(i,j,coor_arr[index][0],coor_arr[index][1]) < half_track_width:
                binarization_arr[index][i][j] = 1
                #if index == 0:
                #    print("+")
    ...
    with mp_pose.Pose(min_detection_confidence=0.5,min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                #right hand
                right_hand.append([int(results.pose_landmarks.landmark[20].x *w),int(results.pose_landmarks.landmark[20].y *h)])
                ##left hand
            ...
            image = draw_circle(img,right_hand)
