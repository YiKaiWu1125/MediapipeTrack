...
with mp_pose.Pose(min_detection_confidence=0.5,min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        if not success:
            ...
            last_x = rx[rindex]-1
            last_y = ry[rindex]
            cv2.rectangle(image,(rx[rindex]-1,ry[rindex]-1),(rx[rindex]+1,ry[rindex]+1),(255,255,0),5)   # 畫出觸碰區
            ...
            while True:
                ...
                for i in range(index_len):
                    if i != rindex and (last_x != rx[i] or last_y != ry[i]) and val >= line_length(rx[rindex],ry[rindex],rx[i],ry[i]) and rearr[i]:
                ...
                if box_index[0] == nowindex :
                    cv2.line(image,(rx[rindex],ry[rindex]),(rx[nowindex],ry[nowindex]),(255,0,255),5)
                    print("index : "+ str(nowindex)+ " x: "+ str(rx[nowindex]) + " y: "+str(ry[nowindex]))
                    print("go end .")
                    break
                if rearr[nowindex] == False:
                    print("error")
                    break
                else:
                    cv2.line(image,(rx[rindex],ry[rindex]),(rx[nowindex],ry[nowindex]),(255,0,255),5)
